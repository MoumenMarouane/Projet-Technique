...
export class FacturesService {
  ...
  generer(commandeId: string, montantHt: number, tva: number) {
    ...
    return this.prisma.facture.create({
      data: { commandeId, montantHt, tva, montantTtc },
    });
  }
 ...
 findAll(userId: string) {
  ...
  async addPaiement(factureId: string, dto: CreatePaiementDto) {
    ...
    const totalVerse = facture.paiements.reduce(
      (sum, p) => sum + Number(p.montantVerse),
      0,
    );
    const reste = Number(facture.montantTtc) - totalVerse;
    if (dto.montantVerse > reste)
      throw new BadRequestException(
        'Montant depasse le reste a payer: ' + reste,
      );
    ...
    const statut =
      nouveauTotal >= Number(facture.montantTtc) ? 'SOLDE' : 'PARTIEL';
    await this.prisma.facture.update({
      where: { id: factureId },
      data: { statutPaiement: statut as any },
    });
    return paiement;
  }
}
