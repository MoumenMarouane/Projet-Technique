        ...
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-[12px] border transition-colors ${
            view === 'general'
              ...
              : 'border-[#2d3348] text-slate-400 hover:text-slate-200'
          }`}
        >
          Vue générale
        </button>
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-[#2d3348]">
                  {['#', 'Client', 'Articles', 'Statut', 'Date'].map(h => (
              ...
              <tbody>
                {(stats?.commandesRecentes ?? []).map((cmd: any, i: number) => (
                  <tr key={i} className="border-b border-[#1a2035] last:border-0">
                    ...
                    <td className="px-4 py-3 text-slate-300">{cmd.clientId?.slice(0, 8)}...</td>
                ...
                {(!stats?.commandesRecentes?.length) && (
                  <tr><td colSpan={5} className="px-4 py-6 text-center text-slate-600">Aucune commande</td></tr>
                )}
              </tbody>
            </table>
          </div>
